...
function generateZshCompletions(commands: CommandInfo[]): string {
  ...
  const specialCommands = new Set(["config", "install"]);
  const caseBranches = commands
    .filter((c) => !specialCommands.has(c.name))
    .map((c) => {
  ...
  // Build config option flags for _arguments
  const configCmd = commands.find((c) => c.name === "config");
  const configOptLines = (configCmd?.options || [])
    .map((o) => {
      const long = o.long || o.flags.split(",").pop()?.trim() || "";
      return `'${long}[${o.description.replace(/'/g, "'\\''")}]'`;
    })
...
${caseBranches}
        ...
        config)
          local -a config_keys
          config_keys=(${configKeys.map((k) => `'${k}'`).join(" ")})
          _arguments ${configOptLines} '1:key:->config_key'
          if [[ $state == config_key ]]; then
...
function generateBashCompletions(commands: CommandInfo[]): string {
  ...
  const bashSpecialCommands = new Set(["config", "install"]);
  const configOpts = commands.find((c) => c.name === "config")?.options
    .map((o) => o.long || o.flags.split(",").pop()?.trim() || "")
    .join(" ") || "";

  const optionCases = commands
    .filter((c) => !c.takesSnippetArg && c.options.length > 0 && !bashSpecialCommands.has(c.name))
    .map((c) => {
...
_snip_completions() {
  ...
  case "\${COMP_WORDS[1]}" in
    ...
    install)
      ...
      ;;
    config)
      COMPREPLY=( $(compgen -W "${configOpts} ${configKeys.join(" ")}" -- "\${cur}") )
      return 0
      ;;
${optionCases}
    *)
...
function generateFishCompletions(commands: CommandInfo[]): string {
  ...
  lines.push("");
  lines.push("# config key completions");
  for (const key of configKeys) {
    lines.push(
      `complete -c snip -n '__fish_seen_subcommand_from config' -a '${key}' -d 'Config key'`,
    );
  }
